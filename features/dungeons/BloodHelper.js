import Settings from "../../config";
import Vector3 from "../../../BloomCore/utils/Vector3"

import RenderLib from "RenderLib";
import serverRotations from "../utils/serverRotations";
import { calcYawPitch, leftClick, S32PacketConfirmTransaction, snapTo } from "../utils/utils";

const skins = ["", "", "", "", "", "", "", "", "", "", "", "", "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvOTZjM2UzMWNmYzY2NzMzMjc1YzQyZmNmYjVkOWE0NDM0MmQ2NDNiNTVjZDE0YzljNzdkMjczYTIzNTIifX19", "", "", "", "", "", "", "", "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNTZmYzg1NGJiODRjZjRiNzY5NzI5Nzk3M2UwMmI3OWJjMTA2OTg0NjBiNTFhNjM5YzYwZTVlNDE3NzM0ZTExIn19fQ==", "", "", ""];
const EntityArmorStand = Java.type("net.minecraft.entity.item.EntityArmorStand");
const S17PacketEntityLookMove = Java.type("net.minecraft.network.play.server.S14PacketEntity$S17PacketEntityLookMove");

const roomEnterEvent = Java.type("me.odinmain.events.impl.RoomEnterEvent");

let currentRoom = ""
let watcherPhase = 0;
const entities = {}
const campEntities = {}



register(roomEnterEvent, (event) => {
    if (event?.room?.data?.name == 'Blood') {
            renderHelper.register()
            autoCamp.register()
    } else {
        renderHelper.unregister()
        autoCamp.unregister()
    }
})

const autoCamp = register("packetReceived", () => {
    if (!Settings().AutoCamp) return;
    if (!watcherPhase) return;
    for (let uuid in campEntities) {
        let entity = campEntities[uuid]; 

        if (!entity || !entity.final) continue;
    

        entity.ticks = Math.max(0, entity.ticks - 1);
    
        if (entity.ticks === Settings().CampDelay && !entity.swung) {
            let [yaw, pitch] = calcYawPitch({
                x: entity.final[0], 
                y: entity.final[1] + 2, 
                z: entity.final[2]
            });
                
                // snapTo(yaw, pitch)
                // Client.scheduleTask(1, () => {
                //     Player.asPlayerMP().entityLivingBase.func_71038_i()
                // })

            serverRotations.setRotation(yaw, pitch , () => {
                Player.asPlayerMP().entityLivingBase.func_71038_i()
                serverRotations.resetRotation();
            });
    
            ChatLib.chat("Swinging at entity");
            entity.swung = true;
        }
    }
}).setFilteredClass(S32PacketConfirmTransaction);
    
    
    

const renderHelper = register("renderWorld", () => {
    if (!Settings().CampHelper) return;
    for (let entry of Object.entries(entities)) {
        let uuid = entry[0];
        let entityData = entry[1];

        let { final, initial, entity, ticks } = entityData;

        
        // Render box at the predicted final position
        RenderLib.drawEspBox(final[0], final[1] + 1, final[2], 0.75, 0.75, 1, 0, 0, 1, true);
        
            // Compute movement vectors
        let currentToFinal = new Vector3(final[0], final[1], final[2])
            .subtract(new Vector3(entity.getRenderX(), entity.getRenderY(), entity.getRenderZ()));
        
        let initialToFinal = new Vector3(final[0], final[1], final[2])
            .subtract(new Vector3(initial[0], initial[1], initial[2]));
        
        let progress = currentToFinal.getLength() / initialToFinal.getLength();
        

        let estimatedTime = (progress * ticks / 20).toFixed(2) + "s";
        if (!estimatedTime || !final[0] || !final[1] || !final[2]) return; 
        Tessellator.drawString(estimatedTime, final[0], final[1] + 1.5, final[2]);
        
        if (entity.isDead()) {
            delete entities[uuid];
        }
        }
        
        
}).unregister()


register("packetReceived", (packet) => {
    const mcEntity = packet.func_149065_a(World.getWorld());
    if (!(mcEntity instanceof EntityArmorStand)) return;

    const itemStack = mcEntity.func_82169_q(3);
    if (!itemStack) return;

    const heldItem = new Item(itemStack);
    if (heldItem.getID() !== 397) return;

    const nbtObject = heldItem.getItemNBT()?.toObject();
    if (!nbtObject) return;

    const textures = nbtObject.tag?.SkullOwner?.Properties?.textures?.[0]?.Value;
    if (!textures) return;
    if (!skins.includes(textures)) return;

    const entity = new Entity(mcEntity);
    const [x, y, z] = [entity.getX(), entity.getY(), entity.getZ()];
    if (new Vec3i(x, y, z).distance(new Vec3i(-185 + Math.floor((Player.getX() + 200) / 32) * 32, 75, -185 + Math.floor((Player.getZ() + 200) / 32) * 32)) > 20) return;
    const [dx, dy, dz] = [packet.func_149062_c() / 32, packet.func_149061_d() / 32, packet.func_149064_e() / 32];
    const entityUUID = entity.getUUID();

    const e = entities[entityUUID] ??= {
        entity,
        initial: [x, y, z],
        final: [],
        vectors: [],
        ticks: watcherPhase ? 19 : 39
    };

    e.vectors.push(new Vector3(dx, dy, dz));

    const vec = new Vector3(0, 0, 0);
    for (let v of e.vectors) vec.add(v);

    vec.multiply(1 / e.vectors.length * (e.ticks < 20 ? 13.5 : 26.5));

    const finalVec = new Vector3(e.initial[0], e.initial[1], e.initial[2]).add(vec);    
    e.final = [finalVec.getX(), finalVec.getY(), finalVec.getZ()];

    if (campEntities[entityUUID]) {
        campEntities[entityUUID].final = [finalVec.getX(), finalVec.getY(), finalVec.getZ()];
    } else {
        campEntities[entityUUID] = {
            final: [finalVec.getX(), finalVec.getY(), finalVec.getZ()],
            ticks: 41,
            swung : false
        };
    }
        
}).setFilteredClass(S17PacketEntityLookMove)

register("chat", (event) => {
    const chatMessage = ChatLib.getChatMessage(event, false)
    if (["[BOSS] The Watcher: Congratulations, you made it through the Entrance.", "[BOSS] The Watcher: Ah, you've finally arrived.", "[BOSS] The Watcher: Ah, we meet again...", "[BOSS] The Watcher: So you made it this far... interesting.", "[BOSS] The Watcher: You've managed to scratch and claw your way here, eh?", "[BOSS] The Watcher: I'm starting to get tired of seeing you around here...", "[BOSS] The Watcher: Oh.. hello?", "[BOSS] The Watcher: Things feel a little more roomy now, eh?"].includes(chatMessage)) {
        watcherPhase = 0
    } else if (chatMessage === "[BOSS] The Watcher: Let's see how you can handle this.") {
        watcherPhase = 1;
    }

})